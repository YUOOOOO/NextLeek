...
async function selectPage(page:Page){
  ...
  if(page==='仪表盘'){
    ...
    if(dashboard){ try { runtime.value=await props.api.launchPlugin(dashboard.manifest.id) } catch(cause) { error.value=String(cause) } }
  }
}
async function loadPlugins(){ plugins.value = await props.api.listPlugins(); const dashboard=plugins.value.find(plugin=>plugin.manifest.id==='com.nextleek.dashboard'); if(dashboard && !runtime.value){ try { runtime.value=await props.api.launchPlugin(dashboard.manifest.id) } catch(cause) { error.value=String(cause) } } }
...
function currentDraft():GeneratePluginRequest['currentDraft'] { return {manifest:structuredClone(draft),files:structuredClone(creatorFiles.value)} }
