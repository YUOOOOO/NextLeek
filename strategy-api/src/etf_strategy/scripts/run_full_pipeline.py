...
def run_command(cmd: list[str], env: dict = None, check: bool = True):
    ...
    full_env.setdefault("ETF_STRATEGY_ROOT", str(ROOT))
    if env:
        full_env.update(env)
...
def main():
    ...
    if args.with_mining:
        ...
        run_command(
            [
                sys.executable, "-m", "etf_strategy.scripts.run_factor_mining",
    ...
    if extra_factors_dir:
        ...
        logger.info("=" * 80)
        run_command(
            [sys.executable, "-m", "etf_strategy.scripts.precompute_non_ohlcv_factors"],
            env=env_cfg,
        )
    ...
    if not args.skip_wfo:
        ...
        run_command(
            [sys.executable, "-m", "etf_strategy.run_robust_combo_wfo"],
            env=env_cfg,
        )
    else:
    ...
    run_command(
        [
            sys.executable,
            "-m",
            "etf_strategy.scripts.run_full_space_vec_backtest",
            "--combos",
            str(vec_input),
            "--config",
    ...
    run_command(
        [
            sys.executable,
            "-m",
            "etf_strategy.scripts.batch_bt_backtest",
            "--combos",
            str(vec_results),
            "--topk",
            str(args.top_n),
    ...
    rolling_cmd = [
        sys.executable,
        "-m",
        "etf_strategy.scripts.run_rolling_oos_consistency",
        "--config",
        str(effective_config_path),
        "--input",
        str(vec_results),
    ...
    run_command(
        [
            sys.executable,
            "-m",
            "etf_strategy.scripts.run_holdout_validation",
            "--config",
            str(effective_config_path),
            "--training-results",
            str(vec_results),
    ...
    run_command(
        [
            sys.executable,
            "-m",
            "etf_strategy.scripts.final_triple_validation",
            "--vec",
            str(vec_results),
            "--rolling",
            str(rolling_results),
